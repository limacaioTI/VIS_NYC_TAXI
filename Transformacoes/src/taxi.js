...
export class Taxi {
    ...
    async loadTaxi(months = 3) {
        ...
        for (let id = 1; id <= months; id++) {
            ...
            const url = `${this.color}/${this.year}/${this.color}_tripdata_${this.year}-${sId}.parquet`;
            files.push({ key: `Y${this.year}M${sId}`, url: url });

            console.log(`Tentando carregar arquivo: ${url}`);
            try {
                ...
                console.log(`Arquivo ${url} carregado com sucesso! Tamanho: ${arrayBuffer.byteLength} bytes`);
                
                await this.db.registerFileBuffer(files[files.length - 1].key, new Uint8Array(arrayBuffer));
                console.log(`Arquivo ${files[files.length - 1].key} registrado no DuckDB`);
            } catch (error) {
                console.error(`Erro ao processar arquivo ${url}:`, error);
                throw new Error(`Falha ao carregar arquivo ${url}. Verifique se o arquivo existe em data/${url}. Erro: ${error.message}`);
            }
        }

        console.log(`Criando tabela ${this.table} com ${files.length} arquivo(s)...`);
    ...
    async filter(limit = 50) {
        ...
        const sql = `
            SELECT *
            FROM ${this.table}
            WHERE month(tpep_pickup_datetime) = ${month} AND year(tpep_pickup_datetime) = ${year}
            LIMIT ${limit};
        `
        return await this.query(sql);
    }
    ...
    async groupBy(limit = 50) {
        ...
        const sql = `
            SELECT day(tpep_pickup_datetime) as day, count(*) as count
            FROM ${this.table}
            WHERE month(tpep_pickup_datetime) = ${month} AND year(tpep_pickup_datetime) = ${year}
            GROUP BY day
            LIMIT ${limit};
    ...
    async normalize(limit = 50) {
        ...
        const sql = `
            SELECT *,
                   CASE 
                       WHEN (t1.max_fare - t1.min_fare) = 0 THEN 0
                       ELSE (fare_amount - t1.min_fare) / (t1.max_fare - t1.min_fare)
                   END as normalized
            FROM ${this.table},
                 (
